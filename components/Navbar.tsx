...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex flex-col pointer-events-none">
      {/* Top Bar - Dark Blue */}
      <div
        className={`bg-navTop text-white transition-all duration-700 cubic-bezier-[0.4,0,0.2,1] pointer-events-auto overflow-hidden ${scrolled
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12 h-[45px] flex justify-between items-center text-[11px] font-bold tracking-widest">
          <div className={`flex items-center space-x-6 transition-all duration-500 delay-75 ${scrolled ? 'blur-sm scale-95' : 'blur-0 scale-100'}`}>
            <div className="flex items-center space-x-2 group">
              ...
              <span className="hidden sm:inline text-white/90">(+63) 945 612 8186/ 945 612 8190</span>
              <span className="sm:hidden text-white/90">(+63) 945 612 8186</span>
            </div>
      ...
      {/* Main Navbar */}
      <div className={`pointer-events-auto transition-all duration-500 ease-in-out ${scrolled ? 'glass shadow-lg py-1' : 'bg-white py-3'
        }`}>
            ...
            <div className="flex-shrink-0">
              <Link to="/" className="flex items-center group">
                <img
                  src={logoUrl}
                  alt="Company Logo"
                  className={`transition-all duration-500 object-contain group-hover:scale-110 drop-shadow-md ${scrolled ? 'h-10' : 'h-14'
                    }`}
                  onError={(e) => {
            ...
            <div className="hidden md:flex space-x-10 items-center">
              {navLinks.map((link) => (
                <div key={link.name} className="relative group">
                  <Link
                    to={link.path}
                    className={`flex items-center text-[13px] font-bold tracking-widest transition-all duration-300 ${isActive(link.path)
                      ? 'text-brand'
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden absolute top-full left-0 w-full bg-white border-t border-gray-100 shadow-2xl transition-all duration-300 pointer-events-auto ${isOpen
      ...
      >
        <div className="px-6 py-8 space-y-4">
          ...
          <Link
            ...
            className={`block text-lg font-bold ${isActive('/') ? 'text-brand' : 'text-gray-700 hover:text-brand'
              }`}
          >
            HOME
          </Link>
          ...
          <Link
            to="/about"
            onClick={() => setIsOpen(false)}
            className={`block text-lg font-bold ${isActive('/about') ? 'text-brand' : 'text-gray-700 hover:text-brand'
              }`}
          >
            ABOUT
          </Link>
          ...
          <div>
            <button
              ...
              className="w-full flex items-center justify-between text-lg font-bold text-gray-700 hover:text-brand"
            >
              SERVICES
              <ChevronDown
                size={18}
            ...
            <div
              className={`overflow-hidden transition-all duration-300 ${mobileServicesOpen ? 'max-h-40 mt-3' : 'max-h-0'
                }`}
            >
              <div className="pl-4 border-l-2 border-brand space-y-2">
                <Link
                  to="/services"
                  onClick={() => {
                    setIsOpen(false);
                  ...
                  className="block py-2 text-base font-medium text-gray-600 hover:text-brand"
                >
                  Our Expertise
                </Link>
          ...
          <Link
            to="/products"
            onClick={() => setIsOpen(false)}
            className={`block text-lg font-bold ${isActive('/products') ? 'text-brand' : 'text-gray-700 hover:text-brand'
              }`}
          >
            PRODUCTS
          </Link>
          ...
          <Link
            to="/contact"
            onClick={() => setIsOpen(false)}
            className={`block text-lg font-bold ${isActive('/contact') ? 'text-brand' : 'text-gray-700 hover:text-brand'
              }`}
          >
            CONTACT
          </Link>
